...
const token = process.env.BOT_TOKEN || process.env.MAX_BOT_TOKEN;
...
Если ссылка не работает: +79952205567`,
  ...
  performance: `Так как Вы только начали использовать ноутбук, то в первые дни возможны повышенная нагрузка на аккумулятор, нагрев, шум вентилятора.

Рекомендации:
1. Подключите ноутбук к сети на 4-6 часов
2. Не прерывайте обновления
3. Проверьте состояние через 2-3 дня
...
Телефон: +79952205567 (пн-пт 10:00-19:00)`);
});
